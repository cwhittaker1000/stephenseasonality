...
data <- overall %>% # need to figure out whether to do rf_train or data here 
  dplyr::select(cluster, country, monthly_catch, population_per_1km:mean_temperature_driest_quarter, -LC_190, -LC_210, -mean_temperature_driest_quarter) %>% # LC190 is urban so correlated v strong with PopPer1km
...
saveRDS(iterations, file = paste0(here("outputs", "random_forest_outputs", "repeated_rf_noUpsampling_FullData_ftCatchCov.rds")))
saveRDS(iterations_ups, file = paste0(here("outputs", "random_forest_outputs", "repeated_rf_Upsampling_FullData_ftCatchCov.rds")))

# Loading in the fitted random forests and processing accuracy results 
ups <- readRDS(file = here("outputs", "random_forest_outputs", "repeated_rf_Upsampling_FullData_ftCatchCov.rds"))
no_ups <- readRDS(file = here("outputs", "random_forest_outputs", "repeated_rf_noUpsampling_FullData_ftCatchCov.rds"))
...
AUC_upsample_plot_ups <- ggplot(df_ups, aes(x = 1-specificity, y = sensitivity, id = factor(iteration))) +
  geom_path(alpha = 0.5) +
...
new_names_up <- var_names_ups
new_names_up <- gsub("country_", "Study\nfrom\n", new_names_up)
new_names_up <- gsub("C_", "C", new_names_up)
new_names_up <- gsub("precipitation_", "Rain\n", new_names_up)
new_names_up <- gsub("temperature_", "Temp.\n", new_names_up)
new_names_up <- gsub("seasonality*", "Season.", new_names_up)
new_names_up <- gsub("_cv", "", new_names_up)
new_names_up <- gsub("monthly_catch", "Monthly\nCatch", new_names_up)
new_names_up <- gsub("coldest_quarter", "Coldest\nQuarter", new_names_up)
new_names_up <- gsub("population_per_1km", "Popn.\nPer\nKm2", new_names_up)

importance_upsample_plot_ups <- ggplot(importance_upsample, aes(x = reorder(Variable, mean_Importance), y = mean_Importance, 
  ...
  geom_errorbar(aes(ymin = pmax(0, mean_Importance - 1.96 * stdev_Importance),
                ...
                width = 0.5) +
  scale_x_discrete(labels = new_names_up) +
...
rf_plot_ups <- cowplot::plot_grid(AUC_upsample_plot_ups, importance_upsample_plot_ups, nrow = 1, ncol = 2, rel_widths = c(1, 2), align = "h", axis = "b")

# No upsampling
no_ups_AUC_plot <- ggplot(df, aes(x = 1-specificity, y = sensitivity, id = factor(iteration))) +
  geom_path(alpha = 0.5) +
  geom_segment(aes(x = 0, xend = 1, y = 0, yend = 1), col = "black", lty = 2) +
  xlab("1 - Specificity") +
  ylab("Sensitivity") +
...
new_names <- var_names_ups
new_names <- gsub("country_", "Study\nfrom\n", new_names)
new_names <- gsub("C_", "C", new_names)
new_names <- gsub("precipitation_", "Rain\n", new_names)
new_names <- gsub("temperature_", "Temp.\n", new_names)
new_names <- gsub("seasonality*", "Season.", new_names)
new_names <- gsub("_cv", "", new_names)
new_names <- gsub("monthly_catch", "Monthly\nCatch", new_names)
new_names <- gsub("coldest_quarter", "Coldest\nQuarter", new_names)
new_names <- gsub("population_per_1km", "Popn.\nPer\nKm2", new_names)
...
rf_plot_noups <- cowplot::plot_grid(no_ups_AUC_plot, importance_noUps_plot, nrow = 1, ncol = 2, rel_widths = c(1, 2), align = "h", axis = "b")

rf_overall_plot <- cowplot::plot_grid(rf_plot_ups, rf_plot_noups, nrow = 2)
ggsave(filename = here("figures/Supp_Figure_CatchCov_AUC_VIP_Plots.pdf"), plot = rf_overall_plot, width = 14, height = 8)
