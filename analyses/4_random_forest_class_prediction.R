...
data <- overall %>% # need to figure out whether to do rf_train or data here 
  #dplyr::select(peaks, country, population_per_1km:mean_temperature_driest_quarter, -LC_190, -mean_temperature_driest_quarter) %>% # LC190 is urban so correlated v strong with PopPer1km
  dplyr::select(peaks, country, population_per_1km:mean_temperature_driest_quarter, -LC_190) %>% #, -mean_temperature_driest_quarter) %>% # LC190 is urban so correlated v strong with PopPer1km
...
set.seed(915)
rf_split <- initial_split(data, prop = 0.85, strata = peaks)
...
envt_recipe <- recipe(peaks  ~ ., data = rf_train) %>%
  ...
  step_nzv(all_numeric(), freq_cut = 80/20) %>% # remove variables that are sparse and unbalanced - remove where >80% of variable values are same value
  step_corr(all_numeric(), threshold = 0.60) %>%
...
rem <- c("country_India", "country_Iran", "peaks")
...
corr_vector <- as.vector(correlation_matrix)
corr_vector <- corr_vector[corr_vector != 1]
