...
table(rf_train$country)/sum(as.numeric(table(rf_train$country))) # checking balance of countries in each dataset
table(rf_test$country)/sum(as.numeric(table(rf_test$country)))

# Prepping Data - Selecting Variables, Creating Recipe 
set.seed(915)
envt_recipe <- recipe(per_ind_4_months ~ ., data = rf_train) %>%
...
full_forest_pred <- predict(final_random_forest_fit, baked_train)
sqrt(sum((full_forest_pred$.pred - baked_train$per_ind_4_months)^2)/length(full_forest_pred$.pred))
plot(baked_train$per_ind_4_months, full_forest_pred$.pred, pch = 20, xlim = c(0, 1), ylim = c(0, 1))
lines(0:1, 0:1, lty = 3)
cor(baked_train$per_ind_4_months, full_forest_pred$.pred)
