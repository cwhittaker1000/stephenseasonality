...
data <- overall %>% # need to figure out whether to do rf_train or data here 
  dplyr::select(cluster, country, rainfall_seas_3, monthly_catch, -precipitation_seasonality_cv, population_per_1km:mean_temperature_driest_quarter, -LC_190) %>% # LC190 is urban so correlated v strong with PopPer1km
...
for (i in 1:number_iterations) {
  ...
  envt_recipe <- recipe(cluster  ~ ., data = data) %>%
    ...
    step_corr(all_numeric(), -contains(c("population_per_1km", "temperature_seasonality", "rainfall_seas_3", "max_temperature_warmest_month", "monthly_catch")), threshold = 0.50) %>%
    step_dummy(country)
