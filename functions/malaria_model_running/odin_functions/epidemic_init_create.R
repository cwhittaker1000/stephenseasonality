epidemic_init_create <- function(age_vector, het_brackets,
         country = NULL, admin_unit = NULL, ft,
         EIR, model_param_list, density_vec)
{
  ...
  mv0 <- omega * EIRd_eq/(Iv_eq * mpl$av0)#d
  ...
  res <- list(
    ...
    #Those able to acquire clinical malaria
    init_Y = Y_eq, 
    ...
    init_ICA = ICA_eq,
    init_ICM = ICM_eq, 
    ICM_init_eq = ICM_init_eq, 
    
    #Adult mosquito stages
    init_Sv = Sv_eq,
    init_Ev = Ev_eq,
    init_Iv = Iv_eq, 
    
    #Larval mosquito stages
    init_EL = EL_eq,
    init_LL = LL_eq, 
    init_PL = PL_eq, 
    ...
    K0 = K0, na = na, nh = nh, ni = num_int, x_I = x_I, mv0 = mv0,
